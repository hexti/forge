class ModelSummaryExtension extends Autodesk.Viewing.Extension {
  ...
  onToolbarCreated() {
      ...
      this._button.onClick = (ev) => {
          ...
          if (this._panel == null) {
              this._panel = new ModelSummaryPanel(this.viewer, this.viewer.container, 'modelSummaryPanel', 'Arquivos Técnicos');
          }

          const selection = this.viewer.getSelection();
          this.viewer.clearSelection();
          console.log(this.viewer.clearSelection());
          // Anything selected?
          if (selection.length > 0) {
                let isolated = [];
                selection.forEach((dbId) => {
                    this.viewer.getProperties(dbId, (props) => {
                        this._panel.addProperty(props.dbId, props.name, '');
                    });
                });
          }
        //   console.log(this._panel.getProperties())
          this._panel.addProperty('', '<h3>Teste</h3>', '');
        //   this._panel.addProperty('2', 'Teste 2', 'Teste 2');
